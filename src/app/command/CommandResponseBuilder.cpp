...
void CommandResponseBuilder::status(CommandResponse &response, const PowerSnapshot &snapshot) {
    response.status = CommandResponseStatus::OK;
    response.type = CommandResponseType::POWER_STATUS;
    response.error = CommandError::NONE;
    response.payload.power = snapshot;
}
